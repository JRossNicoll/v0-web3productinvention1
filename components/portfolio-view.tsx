...
import { Wallet, TrendingUp, ArrowUpRight, ArrowDownLeft, PieChart } from "lucide-react"
...
export function PortfolioView() {
          ...
          <CardContent className="pt-6 relative z-10">
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-muted-foreground text-base max-w-md mb-10 leading-relaxed font-mono text-sm">
                Link your Ethereum wallet to view holdings, track performance, and manage your human equity portfolio.
              </p>
  ...
  const portfolioAssets = userState.portfolio
    .map((item) => {
  ...
  const VANT_PRICE = 0.0000165

  const vantAsset = {
    id: "$VANT",
    name: "Vantage Token",
    symbol: "$VANT",
    image: "/vantage-token.jpg",
    quantity: realVantBalance,
    avgPrice: VANT_PRICE,
    currentPrice: VANT_PRICE,
    value: realVantBalance * VANT_PRICE,
    change24h: 0,
    pnl: 0,
    pnlPercent: 0,
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="border-white/10 hover:bg-white/5 font-mono text-xs bg-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-white/10 bg-black/40 backdrop-blur-md">
          <CardContent className="p-6">
            <div className="text-[10px] font-bold text-muted-foreground tracking-widest mb-2 font-mono">
              TOTAL BALANCE
            </div>
            ...
            <div className="flex items-center gap-2 mt-2 text-xs font-mono">
              ...
              >
                ...
                {dailyPnL >= 0 ? "+" : ""}
                {((dailyPnL / totalValue) * 100).toFixed(2)}%
              </span>
        ...
        <Card className="border-white/10 bg-black/40 backdrop-blur-md">
          <CardContent className="p-6">
            ...
            <div
              className={`text-4xl font-bold tracking-tight font-mono text-white ${dailyPnL >= 0 ? "text-neon-green" : "text-red-500"}`}
            >
              {dailyPnL >= 0 ? "+" : ""}$
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-white/5 hover:bg-transparent">
                    ...
                    <TableHead className="font-mono text-[10px] tracking-wider text-muted-foreground text-right">
                      BALANCE
                    </TableHead>
                    <TableHead className="font-mono text-[10px] tracking-wider text-muted-foreground text-right">
                      VALUE
                  ...
                  {allAssets.map((asset) => (
                    <TableRow key={asset.id} className="border-white/5 hover:bg-white/5 transition-colors group">
                      <TableCell className="py-4">
                        <Link
                          href={asset.id === "$VANT" ? "#" : `/market/asset/${asset.id}`}
                          className="flex items-center gap-4"
                        >
                          <Avatar className="h-8 w-8 border border-white/10 rounded-none">
                            <AvatarImage
                              src={(asset as any).image || "/placeholder.svg"}
                              alt={asset.name}
                              className="object-cover"
                            />
                            <AvatarFallback className="rounded-none bg-white/5 font-mono text-xs">
                              {asset.name.substring(0, 2)}
                            </AvatarFallback>
                          </Avatar>
                          <div>
                            <div className="font-bold text-sm text-white group-hover:text-neon-blue transition-colors font-mono">
                              {asset.name}
                            </div>
                      ...
                      <TableCell className="text-right w-[200px]">
                        <div className="flex items-center justify-end gap-3">
                          ...
                          <div className="w-16 h-1 bg-white/5 overflow-hidden">
                            <div className="h-full bg-white" style={{ width: `${Math.min(asset.allocation, 100)}%` }} />
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        {asset.change !== 0 && (
                          <span
                            className={`font-mono text-xs ${asset.change > 0 ? "text-neon-green" : "text-red-500"}`}
                          >
                            {asset.change > 0 ? "+" : ""}
